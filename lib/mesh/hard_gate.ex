defmodule Mesh.HardGate do
  @moduledoc """
  Deny-by-default mesh hard gate with operator-only timed bypass.

  Law (ops MESH-HARD-GATE.md):
  - Default ON — CommandFabric allowlist is the mutate surface
  - Audit always (allow | deny | bypass)
  - Agents cannot disable the gate
  ...
  @doc "Current gate view for CLI/status."
  def status do
    st = load_state()
    ...
    %{
      ...
      bypass: bypass,
      note: "gate stays ON; bypass only softens allowlist for the operator within TTL/scope"
    }
  end
  ...
  def start_bypass(opts) when is_list(opts) do
    ...
    if actor != :operator do
      _ = audit(%{"event" => "bypass_refuse", "actor" => to_string(actor), "reason" => "agents_cannot_disable_gate"})
      {:error, :agents_cannot_disable_gate}
    else
  ...
  Use start_bypass/1. Agents always get :agents_cannot_disable_gate.
  """
  def request_disable(actor \\ :agent) do
    _ =
      audit(%{
        ...
        "reason" => if(actor == :operator, do: "use_timed_bypass", else: "agents_cannot_disable_gate")
      })
  ...
  defp state_path do
    System.get_env("MESH_GATE_STATE") ||
      Path.expand("~/.local/share/mesh/gate.json")
  end

  defp audit(event) when is_map(event) do
    Mesh.Audit.record(Map.put(event, "component", "hard_gate"))
  end
