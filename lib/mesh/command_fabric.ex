defmodule Mesh.CommandFabric do
  ...
  @allowed_commands ~w(health_check restart_service tailscale_status help syncthing_status ensembly_status ensembly_channel_ir)a
  ...
  @doc """
  Normalize + hard-allow. opts: `:actor` (`:operator` | `:agent`), `:host`.
  ...
  def authorize(command, opts \\ []) do
    ...
    host = Keyword.get(opts, :host, Mesh.HardAllow.hostname())

    case do_normalize(command) do
      {:ok, cmd} ->
        Mesh.HardAllow.record_decision(:allow, %{
      ...
      {:error, {:not_allowlisted, cmd}} = err ->
        if Mesh.HardAllow.bypass_covers?(actor, cmd, host) do
          atom = coerce_command_atom(cmd)

          Mesh.HardAllow.record_decision(:bypass, %{
            command: to_string(cmd),
            actor: "operator",
            host: host,
            bypass: "operator"
          ...
          Mesh.HardAllow.record_decision(:deny, %{
            command: to_string(cmd),
            actor: to_string(actor),
            host: host,
  ...
  @doc """
  Read-only ensembly runtime status via local CLI (PATH or ~/.local/bin/ensembly).
  """
  def ensembly_status(_args) do
    case ensembly_cmd(["runtime", "status", "--json"]) do
      {:ok, out} -> {:ok, Jason.decode!(out)}
      other -> other
    end
  rescue
  ...
  @doc """
  Read-only Channel IR via `ensembly turn --channel` (fail-closed when empty).
  """
  def ensembly_channel_ir(_args) do
    case ensembly_cmd(["turn", "--channel", "--location", "home"]) do
      {:ok, out} -> {:ok, Jason.decode!(out)}
      other -> other
    end
  rescue
  ...
  defp ensembly_cmd(args) do
    bin =
      System.find_executable("ensembly") ||
        Path.expand("~/.local/bin/ensembly")

    if not File.exists?(bin) do
      {:error, :ensembly_cli_missing}
    else
  ...
  def help(_args) do
    {:ok,
     [
       ...
       "syncthing_status: local Syncthing API status (if configured)",
       "ensembly_status: ensembly runtime status --json (client read)",
       "ensembly_channel_ir: ensembly turn --channel (fail-closed)",
       "help: this list"
     ]}
  end
